"""Render the report from completed, audited measurements."""
import json,statistics
from pathlib import Path

P=Path(__file__).resolve().parent
summary=json.loads((P/'summary.json').read_text());groups=summary['kernel_groups']
bench=json.loads((P/'results/benchmark.json').read_text())['results']
heads=[[14,0],[14,6],[27,0],[27,6]]
methods=['favor_plus','centered_favor_plus','sderf','aderf','partition','galerkin']
names={'favor_plus':'FAVOR+','centered_favor_plus':'均值校正FAVOR+','sderf':'SDERF','aderf':'ADERF','partition':'新分区kernel','galerkin':'训练Galerkin（有符号）'}

def fmt(x):return f'{x:.4f}' if x<10 else f'{x:.3g}'
def group(ds,h,method,m=64):return next(r for r in groups if r['dataset']==ds and r['head']==h and r['method']==method and r['m']==m)
def table(headers,rows):return '\n'.join(['| '+' | '.join(headers)+' |','|'+'|'.join(['---']*len(headers))+'|']+['| '+' | '.join(map(str,r))+' |' for r in rows])
def kernels_table(ds):
    return table(['head']+[names[m] for m in methods],[[f'L{h[0]}H{h[1]}']+[fmt(group(ds,h,m)['median']) for m in methods] for h in heads])

text=r'''本轮已完成用户要求的同协议kernel对比、可调用Galerkin特征构造与计算量评估。**在这四个真实Qwen head及指定留出协议上，m=64新分区正kernel的raw-kernel误差优于本轮FAVOR+、均值校正FAVOR+、SDERF、ADERF的全部五个测试种子；但它明显更贵。Galerkin的乘积分布kernel误差更低，却因负kernel项及分母抵消，在真实attention块上出现输出不稳定。**
...
for h in heads:
    for method in methods[:4]:
        g=group('internal',h,method)
        seedrows.append([f'L{h[0]}H{h[1]}',names[method],fmt(g['mean']),f"[{fmt(g['minimum'])}, {fmt(g['maximum'])}]"])
pooledrows=[]
for ds in ['internal','official']:
    for h in heads:
        pooledrows.append([ds,f'L{h[0]}H{h[1]}',fmt(group(ds,h,'favor_plus',640)['median']),fmt(group(ds,h,'partition')['median']),fmt(group(ds,h,'galerkin')['median'])])
attention=json.loads((P/'results/internal_same_document.json').read_text())['results']
attrows=[]
for h in heads:
    f=[r['output_nmse'] for r in attention if r['head']==h and r['method']=='favor_plus' and r['m']==64]
    p=next(r for r in attention if r['head']==h and r['method']=='partition')
    g=next(r for r in attention if r['head']==h and r['method']=='galerkin')
    attrows.append([f'L{h[0]}H{h[1]}',fmt(statistics.median(f)),fmt(p['output_nmse']),fmt(g['output_nmse']),f"{100*g['negative_kernel_fraction']:.2f}%",f"{100*g['nonpositive_denominator_fraction']:.4f}%"])
timerows=[]
for method,m in [(x,64) for x in methods]+[('favor_plus',640)]:
    a=next(r for r in bench if r['method']==method and r['m']==m and r['n']==2048 and r['dtype']=='float32')
    b=next(r for r in bench if r['method']==method and r['m']==m and r['n']==2048 and r['dtype']=='float64')
    timerows.append([names[method],m,f"{a['total_dominant_flops_per_query_key_pair']*4*2048/1e9:.3f}",f"{a['timing']['total']['median_ms']:.3f}",f"{b['timing']['total']['median_ms']:.3f}"])
substitutions={
    'INTERNAL_TABLE':kernels_table('internal'),'OFFICIAL_TABLE':kernels_table('official'),
    'SEED_TABLE':table(['head','方法','五种子均值','五种子最小—最大'],seedrows),
    'POOLED_TABLE':table(['数据','head','FAVOR+ m=640','新分区m=64','Galerkin m=64'],pooledrows),
    'ATTENTION_TABLE':table(['head','FAVOR+输出误差中位数','新分区输出误差','Galerkin输出误差','Galerkin负kernel条目','Galerkin非正分母行'],attrows),
    'TIMING_TABLE':table(['方法','m','主导GFLOPs','FP32时间/ms','FP64时间/ms'],timerows)}
for key,value in substitutions.items():text=text.replace(key,value)
(P/'REPORT.zh.md').write_text(text)
print('Wrote REPORT.zh.md',len(text),'characters')
