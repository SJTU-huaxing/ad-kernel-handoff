"""Build the research artifact from recorded experiments, without GPU work."""
...
labels={'native':'冻结原模型','ad_shape':'AD 方向消融','ad_full':'AD 完整特征',
 'ad_norm':'AD + 原生 L2（幅度消去）','ad_write':'AD 独立写入幅度',
 'exp_mlp':'同参数正指数 MLP','signed_residual':'同参数带符号残差 MLP'}
splits=['confirm_wiki','confirm_long']
def ppl(f,k,sp):return next(r for r in s['ppl'] if r['family']==f and r['kind']==k and r['split']==sp)
text=[r'''**幅度—方向结构接入原生 GLA / Gated DeltaNet：局部适配实验**
...
|---|---|---:|---:|''']
for f in ['gla','gdn']:
 kinds=['native']+(['ad_shape','ad_full'] if f=='gla' else ['ad_norm','ad_write'])+['exp_mlp','signed_residual']
 for k in kinds:
  vals=[]
  for sp in splits:
   r=ppl(f,k,sp);vals.append(f"{r['ppl_mean']:.5f}"+(f" ± {r['ppl_seed_std']:.5f}" if k!='native' else ''))
  text.append(f"| {f.upper()} | {labels[k]} | {' | '.join(vals)} |")
text.append(r'''
...
|---|---|---:|---:|''')
for f in ['gla','gdn']:
 ad='ad_full' if f=='gla' else 'ad_write';direction='ad_shape' if f=='gla' else 'ad_norm'
 for other in ['native',direction,'exp_mlp','signed_residual']:
  vals=[]
  for sp in splits:
   r=next(r for r in s['paired'] if r['family']==f and r['split']==sp and r['comparison']==f'{ad} minus {other}')
   lo,hi=r['relative_ppl_change_95ci_pct'];vals.append(f"{r['relative_ppl_change_pct']:+.2f} [{lo:+.2f}, {hi:+.2f}]")
  text.append(f"| {f.upper()} | {labels[other]} | {' | '.join(vals)} |")
text.append(r'''
...
|---|---|---:|---:|''')
for f in ['gla','gdn']:
 rows=read(P/'results'/f'{f}_geometry.json')['rows']
 for name in dict.fromkeys(r['name'] for r in rows):
  rr=[r for r in rows if r['name']==name];kind='native' if name=='native' else name[len(f)+1:-4]
  text.append(f"| {f.upper()} | {labels[kind]} | {statistics.mean(r['mean_key_cosine'] for r in rr):.4f} | {statistics.mean(r['associative_recall_nmse'] for r in rr):.4f} |")
text.append(r'''
...
|---|---|---:|---:|---:|''')
for f in ['gla','gdn']:
 tt=read(P/'results'/f'balanced_timing_{f}.json')['summary']
 for row in tt:
  name=row['name'];k='native' if name=='native' else name[len(f)+1:-4]
  text.append(f"| {f.upper()} | {labels[k]} | {row['prefill_ms']:.2f} | {row['decode_ms']:.2f} | {row['state_bytes']/2**20:.3f} |")
text.append(r'''
...
''')
for f in ['gla','gdn']:
 op=read(P/'checks'/f'{f}_operators.json');cache=read(P/'checks'/f'{f}_cache.json')
 assert op['passed'] and cache['passed']
 text.append(f"- {f.upper()}：原生及三个主方法、两层的FLA chunk与FP64显式递推相对L2误差最大{max(r['chunk_relative_l2'] for r in op['rows']):.6f}；fused recurrent最大{max(r['recurrent_relative_l2'] for r in op['rows']):.6f}。")
 text.append(f"- {f.upper()}：原生及四种适配seed11、两篇文档的整段前向与128token prefill后逐token缓存输出相比，logits相对L2最大{max(r['logit_relative_l2'] for r in cache['rows']):.6f}，平均分布KL最大{max(r['mean_teacher_kl'] for r in cache['rows']):.6f}；缓存字节不随后续token增加。")
text.append(r'''
...
fig,axs=plt.subplots(1,2,figsize=(10.8,4.5),sharey=True)
for ax,f in zip(axs,['gla','gdn']):
 ad='ad_full' if f=='gla' else 'ad_write';direction='ad_shape' if f=='gla' else 'ad_norm'
 for sp,shift,color,label in [('confirm_wiki',-.12,'#1f77b4','512 tokens'),('confirm_long',.12,'#d97706','2048 tokens')]:
  rows=[next(r for r in s['paired'] if r['family']==f and r['split']==sp and r['comparison']==f'{ad} minus {k}') for k in ['native',direction,'exp_mlp','signed_residual']]
  xx=np.array([r['relative_ppl_change_pct'] for r in rows]);ci=np.array([r['relative_ppl_change_95ci_pct'] for r in rows])
  ax.errorbar(xx,np.arange(4)+shift,xerr=np.array([xx-ci[:,0],ci[:,1]-xx]),fmt='o',capsize=3,color=color,label=label,markersize=5)
 ax.axvline(0,color='#333333',lw=1);ax.grid(axis='x',alpha=.2)
 ax.set_title('GLA: full AD' if f=='gla' else 'GDN: AD write amplitude')
 ax.set_xlabel('AD relative perplexity change (%)\nNegative: AD improves over control')
 ax.set_yticks(range(4),['Frozen native','Direction only','Positive exp MLP','Signed residual MLP'])
 ax.spines[['top','right']].set_visible(False)
axs[0].invert_yaxis();axs[1].legend(loc='upper right',frameon=False)
fig.suptitle('Two-layer adaptation of frozen native language models',fontsize=13)
fig.text(.5,.015,'95% paired document bootstrap intervals, conditional on the two fitted seeds; reused heldout splits.',ha='center',fontsize=8)
fig.tight_layout(rect=[0,.055,1,.96])
for ext in ['png','pdf','svg']:fig.savefig(P/'results'/f'paired_ppl.{ext}',dpi=180,bbox_inches='tight')
plt.close(fig)
check=read(P/'checks/theory.json');assert check['passed']
assert len(list((P/'fits').glob('*.json')))==16
assert len(list((P/'results').glob('ppl_*.json')))==54
assert read(P/'checks/tokenizer.json')['passed']
artifact_files=sorted(p for folder in ['checks','fits','results'] for p in (P/folder).glob('*.json'))
artifact_files += sorted(P.glob('*.py'))+sorted(P.glob('*.md'))
hashes={str(p.relative_to(P)):hashlib.sha256(p.read_bytes()).hexdigest() for p in artifact_files if p.name!='artifact_hashes.json'}
(P/'artifact_hashes.json').write_text(json.dumps(hashes,indent=2))
print('Report, figure and artifact hashes saved; final audit passed.')
