"""Produce the Chinese experiment report from saved measurements."""
import json
from pathlib import Path
import numpy as np

P=Path(__file__).resolve().parent
s=json.loads((P/'summary.json').read_text());rows=s['aggregated_rows']

def values(ds,method,metric='seed_mean',source='product'):
    return [r[metric] for r in rows if r['dataset']==ds and r['source']==source and r['m']==64 and r['method']==method]

def table(ds,methods):
    header='| head | '+' | '.join(label for _,label in methods)+' |\n'
    header+='|---|'+'---|'*len(methods)+'\n'
    arrays=[values(ds,key) for key,_ in methods]
    for h,label in enumerate(['L14H0','L14H6','L27H0','L27H6']):
        header+='| '+label+' | '+' | '.join(f'{a[h]:.4g}' for a in arrays)+' |\n'
    return header

report=r'''本轮判定：**当前“covariance 适配的正指数 quadrature＋单遍训练”实现 No-Go；没有兑现测试矩阵 NMF 展示出的精度空间。** 这个判定针对本轮函数构造、初始化、损失和优化设置的组合，不是对所有正特征、KAN 或 linear attention 的不可能性结论。
...
'''
report+=table('official',[('NMF','测试矩阵NMF'),('favor_orf','FAVOR+'),('learned_shared','shared，KL'),('learned_untied','untied，KL'),('beta15_shared','shared，β=1.5')])
report+='\n新增内部测试文档，m=64：\n\n'
report+=table('internal',[('NMF','测试矩阵NMF'),('favor_orf','FAVOR+'),('learned_shared','shared，KL'),('learned_untied','untied，KL'),('beta15_shared','shared，β=1.5')])
report+=r'''
